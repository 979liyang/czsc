...
# -*- coding: utf-8 -*-
"""
使用自定义 Tushare 分钟权限接口（stk_mins）批量采集上证/深证股票分钟数据并保存到本地 parquet。

要求：
- 入参与 scripts/fetch_tushare_minute_data.py 保持一致
- 存储结构：--freq 1min 存 raw/minute_by_stock；--freq 60min 存 raw/minute_60_by_stock；其余周期同 1min。
  路径示例：.stock_data/raw/minute_by_stock/stock_code={ts_code}/year={year}/{ts_code}_{year}-{month:02d}.parquet
...
使用案例：
1）先小规模验证（推荐）
    python scripts/minute/fetch.py --freq 1min --limit 10

   python scripts/minute/fetch.py --freq 60min --start-date "2018-01-01 09:00:00" --end-date "2026-02-13 19:00:00" 

3）采集指定股票（逗号分隔）
    python scripts/minute/fetch.py --freq 1min --stocks 600078.SH,000001.SZ

4）指定日期范围（支持两种格式）
    - YYYYMMDD：内部会转换为 stk_mins 的 datetime 字符串
    - YYYY-MM-DD HH:MM:SS：原样传给 stk_mins
    python scripts/minute/fetch.py --freq 1min --start-date 20240101 --end-date 20240105 --limit 10
    python scripts/minute/fetch.py --freq 1min --start-date "2018-01-01 09:00:00" --end-date "2026-02-02 19:00:00" --stocks 600066.SH
    python scripts/minute/fetch.py --freq 1min --stocks 600078.SH,000001.SZ --start-date 2018-01-01 --end-date 2026-02-13           


5）覆盖模式（不与本地已有数据合并去重）
    python scripts/minute/fetch.py --freq 1min --no-incremental --limit 10

6）控制频率（避免被限速/封禁）
    python scripts/minute/fetch.py --freq 1min --sleep 0.2 --limit 10

7）断点续跑（从某只股票之后继续）
    # 跳过 000615.SZ，从下一只股票开始继续采集
    python scripts/minute/fetch.py --freq 1min --resume-after 000615.SZ
    python scripts/minute/fetch.py --freq 1min --start-date "2018-01-01 09:00:00" --end-date "2026-02-02 19:00:00" --resume-after 300161.SZ

8）断点续跑（自动 checkpoint）
    # 每处理成功一只股票就写入 checkpoint；下次带 --checkpoint 会自动从上次成功的股票之后继续
    python scripts/minute/fetch.py --freq 1min --checkpoint

9）从缺失股票列表批量获取
    # 先运行 stock_minute_scan.py 生成缺失列表，然后批量获取
    python scripts/minute/fetch.py --freq 1min --from-missing-list .stock_data/metadata/missing_stocks_20240204_160000.txt

    python scripts/fetch_limit.py --freq 1min --from-missing-list .stock_data/metadata/missing_stocks_20260213_161047.txt --start-date "2018-01-01 09:00:00" --end-date "2026-02-12 19:00:00"

    python scripts/fetch_limit.py --freq 1min --from-missing-list .stock_data/metadata/missing_stocks_20260209_204304.txt --start-date "2018-01-01 09:00:00" --end-date "2026-02-10 19:00:00"
...
- 1min：.stock_data/raw/minute_by_stock/stock_code=600078.SH/year=2024/600078.SH_2024-01.parquet
- 60min：.stock_data/raw/minute_60_by_stock/stock_code=600078.SH/year=2024/600078.SH_2024-01.parquet
- parquet 内字段（至少包含）：
...
def _default_checkpoint_path(freq: str) -> Path:
    """默认 checkpoint 路径（按 freq 区分）"""
    d = project_root / ".stock_data" / ".checkpoints"
    d.mkdir(parents=True, exist_ok=True)
    return d / f"fetch_stk_mins_{freq}.txt"
...
def _get_latest_timestamp(stock_code: str, period: int, minute_subdir: str = "minute_by_stock") -> Optional[pd.Timestamp]:
...
def _minute_subdir(freq: str) -> str:
    """1min 存 minute_by_stock，60min 存 minute_60_by_stock，其余同 1min。"""
    return "minute_60_by_stock" if _period_minutes(freq) == 60 else "minute_by_stock"
...
def _calc_fetch_range(
    ...
    last_ts = _get_latest_timestamp(stock_code, period=period, minute_subdir=_minute_subdir(freq))
    if last_ts is None:
        return edt, edt
...
def _save_minute_df(df: pd.DataFrame, stock_code: str, incremental: bool, freq: str) -> bool:
    """按年月分组保存分钟数据到 RawDataStorage；1min 存 minute_by_stock，60min 存 minute_60_by_stock。"""
    ...
    for (y, m), g in df.groupby(["year", "month"]):
        ...
        ok = storage.save_minute_data_by_stock(
            ...
            incremental=incremental, minute_subdir=subdir
        )
        ok_any = ok_any or ok
    return ok_any
...
def fetch_one_stock(pro, ts_code: str, freq: str, incremental: bool, sdt: Optional[str], edt: Optional[str]) -> bool:
    ...
    if df is None or len(df) == 0:
        ...
        return True
    ok = _save_minute_df(df, stock_code=stock_code, incremental=incremental, freq=f)
...
def build_stock_list(pro, stocks_arg: Optional[str], limit: Optional[int], missing_list_path: Optional[str] = None) -> List[str]:
...
def main():
    ...
    pro = build_pro(token)
    stocks = build_stock_list(pro, stocks_arg=args.stocks, limit=args.limit, missing_list_path=args.from_missing_list)
...
def _parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="Tushare stk_mins 分钟数据批量采集")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="从缺失股票列表文件读取股票代码（由 stock_minute_scan.py 生成），如 .stock_data/metadata/missing_stocks_20240204_160000.txt",
    )
...
def _run_batch(pro, stocks: List[str], args, ckpt_path: Optional[Path]) -> None:
    ...
    for i, ts_code in enumerate(stocks, 1):
        try:
            ok = fetch_one_stock(pro, ts_code, args.freq, args.incremental, args.start_date, args.end_date)
