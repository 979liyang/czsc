...
# -*- coding: utf-8 -*-
"""
检测本地股票**日线**数据：仅校验每年**首条日期**是否为该年首个交易日；不对则拉取当年数据补齐。

当存在 .stock_data/metadata/stock_basic.csv 时，会读取每只股票的 list_date（上市日）：
期望数据范围为 max(20180101, list_date)～昨日，上市日之前年份不要求补位；上市当年首条为 max(当年首交, list_date)。
...
存储结构（与 fetch_tushare_daily.py 一致）：
  .stock_data/raw/daily/by_stock/stock_code={ts_code}/{ts_code}_{year}.parquet

使用示例：
  # 仅导出每年每月首个交易日（单列 start_yyyymmdd）
  python scripts/check_daily_stock.py --export-calendar

  # 检测指定股票：开始时间不对的年份会列出，加 --fetch 则拉取当年数据
  python scripts/check_daily_stock.py --stocks 600066.SH
  python scripts/check_daily_stock.py --stocks 600066.SH --fetch

  # 扫描所有股票，开始时间不对的年份直接补拉
  python scripts/check_daily_stock.py --scan --fetch

  # 从某只股票后面开始（跳过该股票及之前的）
  python scripts/check_daily_stock.py --scan --fetch --resume-after 600066.SH

  # 补拉「最后交易日」到目标结束日：从本地 parquet 推导最后日期；可指定 --end-date，不指定则默认昨日
  python scripts/check_daily_stock.py --catch-up --stocks 600066.SH
  python scripts/check_daily_stock.py --catch-up --stocks 600079.SH --end-date 20260213
  python scripts/check_daily_stock.py --catch-up --scan --end-date 20260213
"""
...
RE_DAILY_YEAR = re.compile(r"_(\d{4})\.parquet$")
...
def _first_bar_date_from_parquet(parquet_path: Path) -> Optional[str]:
    """从 parquet 读取首条日期，返回 YYYYMMDD；日线优先用 date 列。失败返回 None。"""
    try:
        import pandas as pd
        df = pd.read_parquet(parquet_path, columns=None)
        if df is None or len(df) == 0:
            return None
        for c in ("date", "timestamp", "datetime", "dt", "trade_time"):
            if c in df.columns:
...
def _last_bar_date_from_parquet(parquet_path: Path) -> Optional[str]:
    """从 parquet 读取末条日期，返回 YYYYMMDD；日线优先用 date 列。失败返回 None。"""
    try:
        import pandas as pd
        df = pd.read_parquet(parquet_path, columns=None)
        if df is None or len(df) == 0:
...
def get_stock_last_bar_date(base_path: Path, stock_code: str) -> Optional[str]:
    """
    从本地 parquet 扫描该股票所有年份文件，返回最后一条的日期 YYYYMMDD；
    无文件或失败返回 None。
    """
...
def build_first_trading_days_from_reference(
    ...
    """
    从参考数据推导每月首个交易日：优先日线 parquet（000001.SZ），其次 csv_output/000001.SZ。
    返回 (year, month) -> start_yyyymmdd；无数据的月份不包含。
    """
    out = {}
    # 1) 尝试 .stock_data 日线 parquet（按年存，需读入后按月份取每月首日）
    ref_dir = base_path / "raw" / "daily" / "by_stock" / "stock_code=000001.SZ"
    if ref_dir.exists():
        import pandas as pd
        for fp in sorted(ref_dir.glob("*.parquet")):
            match = RE_DAILY_YEAR.search(fp.name)
            if not match:
                continue
            y = int(match.group(1))
            try:
                ...
                for mo in range(1, 13):
                    mask = (df["date"].dt.year == y) & (df["date"].dt.month == mo)
                    part = df.loc[mask, "date"]
                    if len(part) > 0:
                        first_d = part.min().strftime("%Y%m%d")
                        out[(y, mo)] = first_d
            except Exception:
                continue
        if out:
            logger.debug(f"从日线 parquet 000001.SZ 推导 {len(out)} 个月的首个交易日")
            return out
    # 2) 尝试 csv_output
...
def _list_daily_year_files(base_dir: Path) -> List[Path]:
    """日线目录下为按年 parquet：*.parquet，无 year 子目录。"""
...
def _parse_year_month_from_path(file_path: Path) -> Optional[Tuple[int, int]]:
    """日线文件名为 {stock_code}_{year}.parquet，解析出 (year, 1)。"""
    m = RE_DAILY_YEAR.search(file_path.name)
    if m:
...
def get_parquet_path(base_path: Path, stock_code: str, year: int, month: int) -> Path:
    """某股票某年 parquet 路径（日线按年存，month 忽略）。"""
    return (
        base_path
        / "raw"
        / "daily"
...
def check_stock_start_dates(
    ...
    """
    只检测每年开始时间是否正确：已有文件的年份若首条日期 != 该年首个交易日（或上市日），则视为需补拉。
    若提供 list_date（上市日），期望范围为 max(20180101, list_date)～昨日，且上市当年首条为 max(当年首交, list_date)。
    :return: (existing_ym_set, expected_ym_set, need_fetch_list)，need_fetch_list 每项为 ((y,1), actual_first_or_none)。
    actual_first 有值时表示该年有文件但首日不对；None 表示整年缺失。
    """
    base_dir = base_path / "raw" / "daily" / "by_stock" / f"stock_code={stock_code}"
    ...
    # 日线按年存，expected 中同一年只保留 (y, 1) 一次，避免重复检查
    seen_years: Set[int] = set()
    for (y, m) in sorted(expected):
        if y in seen_years:
            continue
        seen_years.add(y)
        expected_first = _effective_first_of_month(y, 1, first_day_calendar, list_date)
        if not expected_first:
            need_fetch.append(((y, 1), None))
            continue
        parquet_path = get_parquet_path(base_path, stock_code, y, 1)
        if not parquet_path.exists():
            need_fetch.append(((y, 1), None))
            continue
        actual_first = _first_bar_date_from_parquet(parquet_path)
        if actual_first is None or actual_first != expected_first:
            need_fetch.append(((y, 1), actual_first))
    return existing, expected, need_fetch


def list_stocks_from_dirs(base_path: Path) -> List[str]:
    root = base_path / "raw" / "daily" / "by_stock"
    if not root.exists():
        return []
    stocks = []
...
def run_catch_up_fetch(
    ...
    """
    拉取日线 [start_yyyymmdd, end_yyyymmdd] 区间数据（从「最后交易日」的下一天到当前最新）。
    :return: (是否成功, 子进程 stderr 文本，用于识别限频等失败原因)
    """
    cmd = [
        sys.executable,
        str(script_path),
    ...
    try:
        ...
        stderr = (ret.stderr or "") + (ret.stdout or "")
...
def run_fetch_for_year(
    ...
    否则（整年缺失或首日错乱）拉整年 [应有首日, min(年末最后交易日, 昨日)]。
    若有 list_date，上市当年应有首日为 max(当年首交, list_date)。
    """
    start_yyyymmdd = _effective_first_of_month(
        year, 1, first_day_calendar, list_date
    ) or first_day_calendar.get((year, 1))
    ...
    if actual_first and actual_first > start_yyyymmdd:
        ...
        year_last_trading = last_trading_day_of_month(year, 12)
        end_yyyymmdd = min(year_last_trading, yesterday)
    cmd = [
        sys.executable,
        str(script_path),
        "--stocks", stock_code,
    ...
    if sleep and sleep > 0:
        cmd.extend(["--sleep", str(sleep)])
    logger.info(f"补拉日线 {stock_code} {year} [{start_yyyymmdd}～{end_yyyymmdd}]")
    try:
        ret = subprocess.run(cmd, cwd=str(project_root), timeout=600)
        return ret.returncode == 0
    except subprocess.TimeoutExpired:
        logger.error(f"补拉超时: {stock_code} {year}")
        return False
    except Exception as e:
        logger.error(f"补拉失败: {stock_code} {year} - {e}")
        return False


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="检测本地股票日线每年首条日期是否正确，不对则拉取当年数据补齐",
    )
    parser.add_argument("--stocks", type=str, default=None, help="股票代码逗号分隔")
    parser.add_argument(
        "--scan",
        action="store_true",
        help="扫描 .stock_data/raw/daily/by_stock 下所有股票",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="将需补拉的年写入文件，每行 stock_code,yyyy,mm",
    )
    parser.add_argument(
        "--export-calendar",
        action="store_true",
...
def main() -> int:
    ...
    if args.catch_up:
        ...
        catch_up_attempted = 0  # 实际执行了补拉的股票数
        for stock_code in stocks:
            ...
            if start_yyyymmdd > end_yyyymmdd:
                ...
                continue
            catch_up_attempted += 1
            ok = run_catch_up_fetch(
                stock_code, start_yyyymmdd, end_yyyymmdd, FETCH_SCRIPT,
                token=args.token, sleep=args.sleep,
            )
            if not ok:
                logger.warning(f"尾段补拉未成功: {stock_code}")
                catch_up_failed.append(stock_code)
        if catch_up_failed:
            logger.warning(f"补位失败共 {len(catch_up_failed)} 只: {', '.join(catch_up_failed)}")
        # 直接输出汇总：一共多少需补位、已补位几只、失败几只及失败列表
        catch_up_ok = catch_up_attempted - len(catch_up_failed)
        print()
        print(f"一共 {catch_up_attempted} 只股票需补位，已补位 {catch_up_ok} 只，失败 {len(catch_up_failed)} 只。")
        if catch_up_failed:
            print("失败列表如下：")
            for code in catch_up_failed:
                print(f"  {code}")
        return 0
    ...
    for stock_code in stocks:
        ...
        if not need_fetch:
            logger.info(f"{stock_code}: 每年开始时间均正确（共 {len(expected)} 月范围）")
            continue
        if not existing:
            logger.info(f"{stock_code}: 无本地数据，将补拉 {len(need_fetch)} 年")
        else:
            logger.warning(
                f"{stock_code}: 共 {len(need_fetch)} 年需补拉（开始时间不对或缺失）: "
                f"{[str(y) for (y, m), _ in need_fetch]}"
            )
        ...
        for (y, m), actual_first in need_fetch:
            ...
            if args.fetch and FETCH_SCRIPT.exists():
                ok = run_fetch_for_year(
                    stock_code, y, FETCH_SCRIPT,
                ...
                if not ok:
                    logger.warning(f"补拉未成功: {stock_code} {y}")
    ...
    if total_need > 0 and not args.fetch:
        logger.info(f"共 {total_need} 年需补拉，可使用 --fetch 拉取当年数据")
    return 0
