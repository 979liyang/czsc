...
- --fetch：检测所有股票（或 --stock 指定一只），对缺失月份及“最后一天到当天”的尾段请求数据

使用示例：
  python scripts/daily/convert.py --stock 000001.SZ

  # 检测所有股票并请求缺失/尾段数据（不指定 --stock 即扫描 daily 下全部）
  python scripts/daily/convert.py --fetch

  # 指定输出目录
  python scripts/daily/convert.py --stock 000001.SZ --output-dir ./csv_output

  # 扁平化输出
  python scripts/daily/convert.py --stock 000001.SZ --flatten
"""
...
def convert_flattened(parquet_files: List[Path], output_dir: Path) -> int:
    ...
    success_count = 0
    for parquet_path in parquet_files:
        # 生成扁平化文件名：year=2024_000001.SZ_2024-01.csv
...
def run_fetch_all(args) -> None:
    """检测所有股票（或 --stock 指定的一只）并请求缺失月份及尾段数据。"""
...
def main():
    ...
    parser.add_argument('--stock', type=str, default='000001.SZ', help='股票代码（如：000001.SZ）；与 --fetch 同用时可选，仅 fetch 时可不传以检测全部')
    ...
    parser.add_argument('--fetch', action='store_true', help='检测所有股票（或 --stock 指定的一只）并请求缺失/尾段数据')
    ...
    if args.fetch:
        run_fetch_all(args)
        if not args.stock:
            logger.info("未指定 --stock，仅执行了检测与请求数据；若需转换 CSV 请指定 --stock 再运行")
            return

    stock_code = (args.stock or "").upper()
    if not stock_code or "." not in stock_code:
        if not args.fetch:
            logger.error("请指定 --stock（如：000001.SZ），或使用 --fetch 检测并请求所有股票数据")
        return
