...
使用示例：
  ...
  python scripts/minute/convert.py --stock 600078.SH

  # 检测所有股票并请求缺失/尾段数据（不指定 --stock 即扫描 minute_by_stock 下全部）
  python scripts/minute/convert.py --fetch

  # 指定输出目录
  python scripts/minute/convert.py --stock 600078.SH --output-dir ./csv_output

  # 扁平化输出
  python scripts/minute/convert.py --stock 600078.SH --flatten
"""
...
def main():
    ...
    parser.add_argument('--stock', type=str, default=None, help='股票代码（如：600078.SH）；与 --fetch 同用时可选，仅 fetch 时可不传以检测全部')
    ...
    if not stock_code or "." not in stock_code:
        if not args.fetch:
            logger.error("请指定 --stock（如：600078.SH），或使用 --fetch 检测并请求所有股票数据")
        return
